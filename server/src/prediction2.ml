...
let get_data (filename : string) : (float list list * float list) =
  ...
  let extract (l : string list) : float list =
    List.tl_exn @@ List.fold ~init: [] ~f: (fun acc elt -> float_of_string elt :: acc) l in
...
let eval (net : Tensor.t -> Tensor.t) (data : (Tensor.t * Tensor.t)) : float = 
  ...
  let rec helper (output : float list) (label : float list) (count : int) : int = 
    match output with 
    | [] -> count 
    | x :: xs -> if int_of_float @@ (round ~dir: `Nearest x) > int_of_float @@ Float.(0.8 * (List.hd_exn label)) && int_of_float @@ (round ~dir: `Nearest x) < int_of_float @@ Float.(1.2 * (List.hd_exn label))
...
let train_and_save () =
  ...
  let ff1 = (Layer.linear vs ~activation: Layer.Relu ~input_dim: 9 128 ~w_init: Zeros) in
  ...
  let ff_out = (Layer.linear vs ~input_dim: 128 1 ~w_init: Zeros) in
  ...
  for index = 1 to 200 do
    ...
    for iter = 0 to 18 do
      ...
      let loss =
        ...
        Tensor.mse_loss (net batch_data) batch_label ~reduction: Torch_core.Reduction.Elementwise_mean 
      in
  ...
  (* Stdio.printf "%s\n" ([%sexp_of : t] model |> Sexp.to_string_hum); *)
  Stdio.printf "%f\n" (Tensor.to_float0_exn (net (Tensor.of_float1 @@ List.to_array [191.09;207.66;183.11;210.79;222.13;271.5;296.52;290.78;349.91])))

let predict (input : float list) : unit =
  let load_model = Sexp.load_sexp "/home/ceash1034/FPSE/CS2_OCaml_Flipper/server/weights.scm" |> t_of_sexp in  
  ...
  let ff1 = (Layer.linear vs ~activation: Layer.Relu ~input_dim: 9 128 ~w_init: (Copy ff1_tensor)) in
  ...
  let ff_out = (Layer.linear vs ~input_dim: 128 1  ~w_init: (Copy ff_out_tensor)) in 
  (* let net data = Layer.forward ff1 data |> Layer.forward ff2 |> Layer.forward ff3 |> Layer.forward ff_out in  *)
  let net data = Layer.forward ff1 data |> Layer.forward ff_out in 
  let layers = Var_store.all_vars vs in 
  let model = extract layers in 
...
let () =
  ...
  | "predict" -> let input = [32.39;19.08;19.55;15.44;15.87;18.19;18.07;18.86;21.81] in predict input 
