...
(* Change this to reflect the directory of the root of the project on your computer, please.*)
let root = "/root/repo/CS2_OCaml_Flipper"
...
let test_yojson_to_steam_item _ =
  assert_equal (yojson_to_steam_item (Yojson.Basic.from_file (root ^ "/server/tests/example_item.json"))) example_item

let test_yojson_to_steam_items _ =
  assert_equal (yojson_to_steam_items [(Yojson.Basic.from_file (root ^ "/server/tests/example_item.json"))]) example_item_list

let test_yojson_to_history _ =
  assert_equal (yojson_to_history [(Yojson.Basic.from_file (root ^ "/server/tests/example_price_point.json"))]) example_history

let test_yojson_to_cs2_items _ =
  assert_equal (yojson_to_cs2_items [(Yojson.Basic.from_file (root ^ "/server/tests/example_cs2_item.json"))]) example_cs2_item_list
...
let test_extract _ = 
  let load_model = Sexp.load_sexp (root ^ "/weights.scm") |> t_of_sexp in 
...
let test_get_data _ = 
  ...
  [1.0; 1.0]) @@ get_data (root ^ "/test.csv")
